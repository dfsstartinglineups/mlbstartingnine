...
async function loadPlayerProfileData() {
    // PLAYER_ID must be defined globally in the parent index.html
    ...
    try {
        ...
        if (masterRes.ok) {
            ...
            if (masterDataProfile) {
                // Route and display correct positional matrix labels based on player type
    ...
    try {
        ...
        if (dailyRes.ok) {
            ...
            if (activeMatchNodes.length > 0) {
                ...
                badgeZone.innerHTML = ''; bvpZone.innerHTML = ''; liveConsoleZone.innerHTML = '';
