 ...
 * Powers all 30 static SEO team pages dynamically without page reloads.
 * ============================================================================
 */

// ==========================================
// 1. MASTER THEME DICTIONARY & CONSTANTS
// ==========================================
const TEAM_THEMES = {
    ...
    110: { name: "Orioles", paperBg: "#f4f1ea", paperLine: "rgba(223, 70, 1, 0.4)", markerInk: "#df4601" },
    111: { name: "Red Sox", paperBg: "#f4f1ea", paperLine: "rgba(189, 48, 57, 0.4)", markerInk: "#bd3039" },
    112: { name: "Cubs", paperBg: "#f4f1ea", paperLine: "rgba(14, 51, 134, 0.4)", markerInk: "#0e3386" },
    113: { name: "Reds", paperBg: "#f4f1ea", paperLine: "rgba(198, 1, 31, 0.4)", markerInk: "#c6011f" },
    114: { name: "Guardians", paperBg: "#f4f1ea", paperLine: "rgba(227, 25, 55, 0.4)", markerInk: "#e31937" },
    115: { name: "Rockies", paperBg: "#f4f1ea", paperLine: "rgba(51, 0, 111, 0.4)", markerInk: "#33006f" },
    116: { name: "Tigers", paperBg: "#f4f1ea", paperLine: "rgba(12, 35, 64, 0.4)", markerInk: "#0c2340" },
    117: { name: "Astros", paperBg: "#f4f1ea", paperLine: "rgba(235, 110, 31, 0.4)", markerInk: "#eb6e1f" },
    118: { name: "Royals", paperBg: "#f4f1ea", paperLine: "rgba(0, 70, 135, 0.4)", markerInk: "#004687" },
    119: { name: "Dodgers", paperBg: "#f4f1ea", paperLine: "rgba(0, 90, 156, 0.4)", markerInk: "#005a9c" },
    120: { name: "Nationals", paperBg: "#f4f1ea", paperLine: "rgba(171, 0, 3, 0.4)", markerInk: "#ab0003" },
    121: { name: "Mets", paperBg: "#f4f1ea", paperLine: "rgba(255, 89, 16, 0.4)", markerInk: "#ff5910" },
    133: { name: "Athletics", paperBg: "#f4f1ea", paperLine: "rgba(0, 56, 49, 0.4)", markerInk: "#003831" },
    134: { name: "Pirates", paperBg: "#f4f1ea", paperLine: "rgba(253, 184, 39, 0.5)", markerInk: "#fdb827" },
    135: { name: "Padres", paperBg: "#f4f1ea", paperLine: "rgba(47, 36, 29, 0.4)", markerInk: "#2f241d" },
    136: { name: "Mariners", paperBg: "#f4f1ea", paperLine: "rgba(0, 92, 92, 0.4)", markerInk: "#005c5c" },
    137: { name: "Giants", paperBg: "#f4f1ea", paperLine: "rgba(253, 90, 30, 0.4)", markerInk: "#fd5a1e" },
    138: { name: "Cardinals", paperBg: "#f4f1ea", paperLine: "rgba(196, 30, 58, 0.4)", markerInk: "#c41e3a" },
    139: { name: "Rays", paperBg: "#f4f1ea", paperLine: "rgba(9, 44, 87, 0.4)", markerInk: "#092c57" },
    140: { name: "Rangers", paperBg: "#f4f1ea", paperLine: "rgba(0, 50, 120, 0.4)", markerInk: "#003278" },
    141: { name: "Blue Jays", paperBg: "#f4f1ea", paperLine: "rgba(19, 74, 142, 0.4)", markerInk: "#134a8e" },
    142: { name: "Twins", paperBg: "#f4f1ea", paperLine: "rgba(0, 43, 92, 0.4)", markerInk: "#002b5c" },
    143: { name: "Phillies", paperBg: "#f4f1ea", paperLine: "rgba(232, 24, 40, 0.4)", markerInk: "#e81828" },
    144: { name: "Braves", paperBg: "#f4f1ea", paperLine: "rgba(206, 17, 65, 0.4)", markerInk: "#ce1141" },
    145: { name: "White Sox", paperBg: "#f4f1ea", paperLine: "rgba(39, 37, 31, 0.4)", markerInk: "#27251f" },
    146: { name: "Marlins", paperBg: "#f4f1ea", paperLine: "rgba(0, 163, 224, 0.4)", markerInk: "#00a3e0" },
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // Get today's date in YYYY-MM-DD format (Pacific / Local time safe)
    const now = new Date();
    ...
    try {
        ...
        // Fallback: Could fetch from statsapi.mlb.com directly if needed
    }
...
// ==========================================
// 3. UI BUILDERS (HEADER, FOOTER, DROPDOWN)
// ==========================================
...
function populateTeamDropdown() {
    ...
    // Extract all playing teams from today's slate
    dailySlateData.games.forEach(game => {
        ...
        ['away', 'home'].forEach(side => {
            ...
            // Map MLB team ID to our slug dictionary
...
function renderTeamPage() {
    ...
    // Apply custom team marker ink colors to CSS Custom Properties
    ...
    // 1. Check if team is playing today
    let targetGame = null;
    let targetSide = null;
    ...
    // 4. BUILD THE VISUAL CARD HTML
    let cardHtml = `
        <div style="max-width: 650px; margin: 20px auto; background: var(--paper-bg); border-radius: 12px; padding: 25px; box-shadow: 0 15px 35px rgba(0,0,0,0.6), inset 0 0 40px rgba(0,0,0,0.05); position: relative; overflow: hidden; border: 1px solid #ddd;">
            
            <img src="https://www.mlbstatic.com/team-logos/${currentTargetId}.svg" style="position: absolute; right: -20px; bottom: -20px; height: 300px; width: 300px; opacity: 0.04; pointer-events: none; z-index: 0;">

            <div style="display: flex; align-items: center; gap: 20px; border-bottom: 2px solid var(--marker-ink); padding-bottom: 15px; margin-bottom: 20px; position: relative; z-index: 1;">
                <img src="https://www.mlbstatic.com/team-logos/${currentTargetId}.svg" alt="${currentTargetName} Logo" style="height: 70px; width: 70px; filter: drop-shadow(0 4px 6px rgba(0,0,0,0.2));">
                <div>
                    ${badgeHtml}
                    <h1 style="font-family: 'Bebas Neue', cursive; font-size: 38px; color: var(--marker-ink); margin: 0; line-height: 0.9; letter-spacing: 1px;">${currentTargetName.toUpperCase()}</h1>
                    <div style="font-family: 'Montserrat', sans-serif; font-size: 14px; color: #333; font-weight: 600; margin-top: 4px;">${vsSymbol} <span style="color: #888; font-weight: 400;">| ${venueName}</span></div>
                    ${oddsStr}
                </div>
            </div>

            <div style="position: relative; z-index: 1;">
                <div style="font-family: 'Montserrat', sans-serif; font-size: 11px; text-transform: uppercase; color: #666; font-weight: 700; letter-spacing: 1px; margin-bottom: 10px; border-bottom: 1px dashed var(--paper-line); padding-bottom: 4px;">Batting Order</div>
    `;

    // Populate Batting Order
    ...
    if (batters.length === 0) {
        ...
        batters.forEach((b, idx) => {
            ...
            cardHtml += `
                <div style="display: flex; align-items: center; justify-content: space-between; padding: 8px 10px; border-bottom: 1px solid var(--paper-line); transition: background 0.15s;" onmouseover="this.style.background='rgba(0,0,0,0.03)'" onmouseout="this.style.background='transparent'">
                    <div style="display: flex; align-items: center; gap: 12px;">
                        <span style="font-family: 'Roboto Mono', monospace; font-size: 14px; font-weight: 700; color: var(--marker-ink); width: 18px;">${idx + 1}.</span>
                        <span style="font-family: 'Montserrat', sans-serif; font-size: 15px; font-weight: 700; color: #111;">${b.name} <span style="font-size: 12px; font-weight: 400; color: #555;">${hand}</span></span>
                    </div>
                    <div style="display: flex; align-items: center; gap: 15px;">
                        <span style="background: rgba(0,0,0,0.06); color: #222; font-family: 'Roboto Mono', monospace; font-size: 11px; font-weight: 700; padding: 2px 6px; border-radius: 4px;">${pos}</span>
                        <span style="font-family: 'Roboto Mono', monospace; font-size: 13px; color: #444; width: 45px; text-align: right;">${sal}</span>
                    </div>
                </div>
            `;
        });
    ...
    const pName = pitcher.name || "TBD";
    const pHand = pitcher.hand ? `(${pitcher.hand}HP)` : "(P)";
    const pSal = pitcher.salary ? `$${pitcher.salary}` : "";

    cardHtml += `
            </div>
            
            <div style="margin-top: 20px; background: rgba(0,0,0,0.04); border-left: 4px solid var(--marker-ink); padding: 12px 15px; border-radius: 0 6px 6px 0; display: flex; justify-content: space-between; align-items: center; position: relative; z-index: 1;">
                <div>
                    <div style="font-family: 'Montserrat', sans-serif; font-size: 10px; text-transform: uppercase; color: #666; font-weight: 700; letter-spacing: 0.5px;">Starting Pitcher</div>
                    <div style="font-family: 'Montserrat', sans-serif; font-size: 16px; font-weight: 700; color: #111; margin-top: 2px;">${pName} <span style="font-size: 13px; font-weight: 400; color: #555;">${pHand}</span></div>
                </div>
                <div style="font-family: 'Roboto Mono', monospace; font-size: 14px; font-weight: 700; color: var(--marker-ink);">${pSal}</div>
            </div>
...
// ==========================================
// 5. DYNAMIC TEAM SWITCHING (NO RELOAD)
// ==========================================
function handleTeamSwitch(selectElement) {
    ...
    // Push new canonical URL to browser history silently!
    ...
    // Repaint the scorecard instantly
    renderTeamPage();
}

// Helper: Map numerical ID back to slug if needed
